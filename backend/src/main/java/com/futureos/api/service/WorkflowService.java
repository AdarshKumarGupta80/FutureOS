...
public class WorkflowService {
    ...
    @Transactional
    public Map<String, Object> advanceMilestone(User user, Long milestoneId) {
        // Use eager-fetch query so roadmap.user is not null (avoids LazyInitializationException / NPE)
        Milestone current = milestones.findByIdWithRoadmapAndUser(milestoneId)
                .orElseThrow(() -> new IllegalArgumentException("Milestone not found: " + milestoneId));
        if (!current.roadmap.user.id.equals(user.id)) {
            throw new IllegalArgumentException("Not your milestone");
        }
        current.status = "DONE";
        milestones.save(current);
        // Fetch siblings by roadmap ID (no lazy traversal needed)
        List<Milestone> all = milestones.findByRoadmapIdOrderById(current.roadmap.id);
        boolean foundCurrent = false;
        for (Milestone m : all) {
            if (foundCurrent && !"DONE".equals(m.status)) {
                m.status = "RUNNING";
                milestones.save(m);
                break;
            }
            if (m.id.equals(milestoneId)) { foundCurrent = true; }
        }
        return dashboard(user);
    }
    ...
    private void saveMilestonesTasksAndExperiments(User user, Roadmap roadmap, Map<String, Object> generated) {
        // Support both new grouped format: milestones:[{title, tasks:[...]}, ...]
        // and old flat format:            milestones:["title", ...] + tasks:["title", ...]
        List<?> rawMilestones = (List<?>) generated.getOrDefault("milestones", List.of("Foundation", "Proof", "Launch"));
        List<Milestone> savedMilestones = new java.util.ArrayList<>();
        // Track per-milestone task lists from the grouped format
        List<List<?>> perMilestoneTasks = new java.util.ArrayList<>();

        int index = 1;
        for (Object entry : rawMilestones) {
            Milestone milestone = new Milestone();
            milestone.roadmap = roadmap;
            milestone.targetDate = LocalDate.now().plusWeeks(index * 2L);

            if (entry instanceof Map<?, ?> milestoneMap) {
                // New grouped format: { "title": "...", "tasks": ["...", "..."] }
                milestone.title = string(milestoneMap.get("title"));
                Object tasksRaw = milestoneMap.get("tasks");
                perMilestoneTasks.add(tasksRaw instanceof List<?> ? (List<?>) tasksRaw : List.of());
            } else {
                // Old flat format: plain string title
                milestone.title = string(entry);
                perMilestoneTasks.add(List.of());
            }

            savedMilestones.add(milestones.save(milestone));
            index++;
        }

        // Mark first milestone as RUNNING so it's immediately actionable
        if (!savedMilestones.isEmpty()) {
            savedMilestones.get(0).status = "RUNNING";
            milestones.save(savedMilestones.get(0));
        }

        // Save tasks: prefer per-milestone grouped tasks; fall back to flat "tasks" list (old format)
        boolean hasGroupedTasks = perMilestoneTasks.stream().anyMatch(t -> !t.isEmpty());
        if (hasGroupedTasks) {
            // New format: each milestone owns its task list
            for (int i = 0; i < savedMilestones.size(); i++) {
                Milestone milestone = savedMilestones.get(i);
                List<?> taskTitles = perMilestoneTasks.get(i);
                for (Object taskTitle : taskTitles) {
                    TaskItem task = new TaskItem();
                    task.user = user;
                    task.milestone = milestone;
                    task.title = string(taskTitle);
                    task.description = "Task for milestone: " + milestone.title;
                    task.commitment = true;
                    task.dueDate = milestone.targetDate;
                    tasks.save(task);
                }
            }
        } else {
            // Old flat format fallback: distribute round-robin across milestones
            List<?> flatTasks = (List<?>) generated.getOrDefault("tasks", List.of("Commit to first weekly action"));
            int milestoneCount = savedMilestones.size();
            int taskIdx = 0;
            for (Object taskTitle : flatTasks) {
                TaskItem task = new TaskItem();
                task.user = user;
                task.title = string(taskTitle);
                ...
                if (milestoneCount > 0) {
                    task.milestone = savedMilestones.get(taskIdx % milestoneCount);
                }
                tasks.save(task);
                taskIdx++;
            }
